...
use yii\easyii\modules\article\models\Item as ArticleModel;
...
class Article extends \yii\easyii\components\API
{
	...
	public function api_popular($limit = 1)
	{
		...
		$result = [];
		foreach(ArticleModel::find()->with($with)->status(ArticleModel::STATUS_ON)->orderBy('views DESC')->limit($limit)->all() as $item){
			$result[] = new ArticleObject($item);
		}
